...
def coverage_and_uniqueness(output, num_classes, num_vc, vc_per_class, top_k):
    ...
    vc_in_class = {i: set() for i in range(num_vc)}                    # {class: {VC: {class1, class2}}}
    ...
    for i in range(num_vc):
        c = (i // vc_per_class)
        coverage[i] = closest_images_per_vc[i] / docs_per_class[c]
        uniqueness[i] = (1 / num_classes) + (1 - (len(vc_in_class[i]) / num_classes)) if len(vc_in_class[i]) > 0 else 0
