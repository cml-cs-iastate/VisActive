...
from src.HVC.Utils.utils import custom_weights_init
from src.HVC.VCL.vcl import VCL


def hvc_vgg(num_classes, image_size, dropout=0.0,  weight_decay=0.0, num_vc=0, p_h=1, p_w=1):
    """
    """
    ...
    fm = layers.MaxPooling2D((2, 2), strides=(2, 2), name='pool4')(x)  # 8 --> 4

    if num_vc > 0:
        x = layers.Conv2D(128, (1, 1), activation='relu', padding='same', name='additional_layer_1')(fm)
        x = layers.Conv2D(128, (1, 1), activation='sigmoid', padding='same', name='additional_layer_2')(x)
        ...
        x = layers.Flatten(name='flatten')(fm)
